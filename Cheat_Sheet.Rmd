...
For our MLR model we could also create a confidence interval for each $\hat{\beta_i}$ using the same method as before. $CI=\hat{\beta_j}\pm t_{(1-\alpha/2;n-p^*)}SE(\hat{\beta_j})$ Where $SE=\hat{\sigma}\sqrt{(X^TX)^{-1}_{jj}}$. If we wanted to test the null hypothesis $H_0:\beta_j=0$, we would fail to reject it if the CI contains zero. Both methods below achieve the same result.
...
When making CIs for predictions, there are typically two types of predictions being made in regression, prediction of response mean for a subject with characteristics $x_0$, and prediction of a future response of a new observation with characteristics $x_0$. A CI for the response mean is: $\hat{y_0}\pm t_{(t-\alpha/2;n-p^*)}\hat{\sigma}\sqrt{x_0^T(X^TX)^{-1}x_0}$. We can use either method below, the first calculates each value using the math, while the other method uses a built in function.
...
A CI for a single future observation is: $\hat{y_0}\pm t_{(1-\alpha/2;n-p^*)}\hat{\sigma}\sqrt{1+x_0^T(X^TX)^{-1}x_0}$. We can once again use either method below.
...
If we wanted to check Homoscedasticity of our model, i.e. plotting the Residuals (vertical axis) vs the fitted values $\hat{y}$ (horizontal axis) we would do the following:
...
Or we could run a Shapiro test with null hypothesis $H_0:$Residuals Are Normal:

```{r}
shapiro.test(residuals(fit1))
```

To check the correlation between our errors, we could plot time or space vs our residuals, and we would expect a random scatter of points above and below $\hat{\epsilon}=0$:
...
If we wanted to calculate the leverage score for every value, or the weighted distance between $X_i$ and the average point of the $X_i 's$. We also know that $\sum_{i=1}^nh_i=p^*=p+1$
...
We can plot these leverage scores to determine which ones might have a very large influence on our model. Anything larger than $2\bar{h}$ is considered a large leverage score.
...
If we wanted to determine outliers in our data, we can calculate standardized residuals: $r_i=\frac{y_i-\bar{y}_i}{\hat{\sigma}\sqrt{1-h_i}}$. Any $|r_i|\geq 3$ is an outlier.
...
We can calculate cook's distance to determine influential observations, or points that when removed would cause significant change in the model. $D_i=\frac{(\hat{y}-\hat{y}_{(i)})^T(\hat{y}-\hat{y}_{(i)})}{p^*\hat{\sigma}^2}=\frac{1}{p^*}r_i^2\frac{h_i}{1-h_i}$
