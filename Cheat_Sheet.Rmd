...
If we are dealing with non-nested models, we have a few criteria for comparing them. The first include the AIC and BIC(k=number of predictors):
...
To see this, we will use the state data set and the regsubsets function to search through all possible combinations of predictors and find the variables for each model size k that produce the minimum SSR. By looking at rs$which, we can see which variables are used in each model size.
...
There are two different step-wise procedures, forward and backward, for eliminating variables to get the best possible model:
